...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    async with db.transaction():
        if (await BagUser.get_gold(event.user_id, event.group_id)) < goods.goods_price * num:
            await buy.finish('您的金币好像不太够哦', at_sender=True)
        if await BagUser.spend_gold(event.user_id, event.group_id, goods.goods_price * num):
            for _ in range(num):
                await BagUser.add_props(event.user_id, event.group_id, goods.goods_name)
            await buy.send(f'花费 {goods.goods_price*num} 金币购买 {goods.goods_name} ×{num} 成功！', at_sender=True)
