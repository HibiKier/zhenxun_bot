import aiohttp
from bilireq import video, live
from .get_b64_image import get_b64_image
from utils.user_agent import get_user_agent

async def parse_bili_url(get_url, information_container):
    """
    解析Bilibili链接，获取相关信息
    ...
    # 根据不同类型的链接进行处理
    if response_url.startswith(("https://www.bilibili.com/video", "https://m.bilibili.com/video/")):
        vd_url = response_url
        vid = vd_url.split("/")[-1]
        ...
        cv_url = response_url
        image_info = await get_b64_image(cv_url)
        information_container.update({"image_info": image_info, "image_url": cv_url})

    elif response_url.startswith(("https://www.bilibili.com/opus", "https://t.bilibili.com")):
        opus_url = response_url
        image_info = await get_b64_image(opus_url)
